...
class GAPI
{
	 ...
	 */
	public function request_account_data($start_index=1, $max_results=20)
	{
		$response = $this->http_request(gapi::account_data_url, array('start-index'=>$start_index,'max-results'=>$max_results), null, $this->generate_auth_header());
		
		if(substr($response['code'],0,1) == '2')
		{
			return $this->account_object_mapper($response['body']);
		}
		else 
		{
	 ...
	 * @param Int $max_results OPTIONAL: Max results returned
	 */
	public function request_report_data($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
	{
		...
		if($filter!=null)
		{
			$filter = $this->process_filter($filter);
			if($filter!==false)
			{
		...
		$response = $this->http_request(gapi::report_data_url, $parameters, null, $this->generate_auth_header());
		
		//HTTP 2xx
		if(substr($response['code'],0,1) == '2')
		{
			return $this->report_object_mapper($response['body']);
		}
		else 
		{
	 ...
	 * @return Array of GAPI_AccountEntry objects
	 */
	protected function account_object_mapper($xml_string)
	{
		...
		foreach($xml->entry as $entry)
		{
			...
			$results[] = new GAPI_AccountEntry($properties);
		}
	 ...
	 *
	 * @param String $xml_string
	 * @return Array of GAPI_ReportEntry objects
	 */
	protected function report_object_mapper($xml_string)
	{
		$xml = simplexml_load_string($xml_string);
		...
		foreach($xml->entry as $entry)
		{
			...
			$results[] = new GAPI_ReportEntry($metrics,$dimensions);
		}
	...
	protected function authenticate_user($email, $password)
	{
		...
		$response = $this->http_request(gapi::client_login_url,null,$post_variables);
	 ...
	 * @param Array $headers
	 */
	protected function http_request($url, $get_variables=null, $post_variables=null, $headers=null)
	{
		...
		if($interface == 'curl')
		{
			return $this->curl_request($url, $get_variables, $post_variables, $headers);
		}
		elseif($interface == 'fopen') 
		{
			return $this->fopen_request($url, $get_variables, $post_variables, $headers);
		}
		else 
		{
	 ...
	 * @param Array $headers
	 */
	private function curl_request($url, $get_variables=null, $post_variables=null, $headers=null)
	{
		$ch = curl_init();
	 ...
	 * @param Array $headers
	 */
	private function fopen_request($url, $get_variables=null, $post_variables=null, $headers=null)
	{
